...
bool display_player_round_score(int round_num, int round_number, int player, bool break_cycle, char **player_scores, int **player_round_score, int **player_total); 
...
int add_player_round_score(int round_num, int round_number, int &player, int spin_total, char **&player_scores, int **&player_round_score, int **&player_total); 
...
int add_player_round_score(int round_num, int round_number, int &player, int spin_total, char **&player_scores, int **&player_round_score, int **&player_total) {
   ...
   if (round_num > 1) {
	 ...
	 player_round_score[player - 1][round_num-1] += spin_total;
      ...
      if (player_scores[player - 1][(round_num-1)*2] == 'X' || player_scores[player - 1][(round_num-1)*2+1] == '/')
	 player_round_score[player - 1][round_num - 1] += spin_total;
   }
   else if (round_num > 0) {
      if (player_scores[player - 1][(round_num - 1)*2] == 'X' || player_scores[player - 1][(round_num-1)*2+1] == '/') {
   ...
   */
   ...
   if (round_num > 0)
      player_round_score[player - 1][round_num] = player_round_score[player - 1][round_num - 1] + spin_total;
   else
      player_round_score[player - 1][round_num] = spin_total;
...
void extra_round(int &round_num, int num_players, int &player, int spin_total, char **&player_scores, int **&player_round_score, int **&player_total, char **player_name) {
   ...
   for (int i = 0; i < num_players; i++) {
	 ...
	 scoresheet(player_name, player_scores, player_total, player_round_score, num_players, round1, player); 

	 cout << "You got an extra turn! Hit enter to bowl.";
	 cin.ignore(INT_MAX, '\n');

	 scoresheet(player_name, player_scores, player_total, player_round_score, num_players, round2, player); 

	 extra_spin_for_bowl(player, round2, num_players, player_scores, player_total, player_round_score, spin_total, strike, spare);

	 if (player_scores[player - 1][20] == 'X')
	    player_round_score[player - 1][9] += spin_total;
	 if (player_scores[player - 1][16] == 'X')
	    player_round_score[player - 1][8] += spin_total;

	 player_round_score[i][9] += spin_total;
	 player_total[i][0] = player_round_score[i][9];

	 //extra_round_scoresheet(player_name, player_scores, player_total, player_round_score, num_players, round2, player); 
	 scoresheet(player_name, player_scores, player_total, player_round_score, num_players, round2, player); 
	 ...
	 //extra_round_scoresheet(player_name, player_scores, player_total, player_round_score, num_players, round3, player); 
	 scoresheet(player_name, player_scores, player_total, player_round_score, num_players, round3, player); 
      }
...
bool display_player_round_score(int round_num, int round_number, int player, bool break_cycle, char **player_scores, int **player_round_score, int **player_total) {
   ...
   if (display_player_round_score(--round_num, round_num, player, break_cycle, player_scores, player_round_score, player_total) && break_cycle) {
...
void scoresheet(char **&player_name, char **&player_scores, int **&player_total, int **&player_round_score, int num_players, int round_num, int player) {
   ...
   for (int i = 0; i < num_players; i++) {
	 ...
	 cout << " |  " << player_scores[i][2*j] << " " << player_scores[i][2*j+1]; 
      ...
      if ( (round_num == 9 && player_scores[i][16] == 'X' || player_scores[i][17] == '/') && (player_scores[i][18] != 'X' && player_scores[i][19] != '/') )
	 player_total[i][0] = player_round_score[i][9];

      if (player_scores[i][18] != 'X' && player_scores[i][18] != ' ' && player_scores[i][19] != '/' && player_scores[i][19] != ' ')
	 player_total[i][0] = player_round_score[i][9];
      ...
      if (round_num == 10 && player_scores[i][19] == '/')
	 goto thing;

      if ( (player_scores[i][20] != 'X' && player_scores[i][20] == ' ' && player_scores[i][21] == ' ') && player_scores[i][21] != '/') {

	 if (i+1 <= player && round_num != 10 && round_num != 11)
	    display_player_round_score(round_num, round_num, i + 1, break_cycle, player_scores, player_round_score, player_total); 
	 else if (round_num == 0)
	    ;
	 else
	    display_player_round_score(round_num - 1, round_num - 1, i + 1, break_cycle, player_scores, player_round_score, player_total); 
      }
      else if ((round_num == 10 || round_num == 11) && (player_scores[i][20] != 'X' && player_scores[i][20] == ' ' || (player_scores[i][21] == ' ')) ) 
	 display_player_round_score(round_num - 1, round_num - 1, i + 1, break_cycle, player_scores, player_round_score, player_total); 
      else {
thing:
	 for (int k = 0; k < 10; k++) {
...
void bowl(int &player, int &round_num, int num_players, char **&player_scores, int **&player_total, int **&player_round_score, char **&player_name) {
   ...
   for (int i = 0; i < 10; i++) {
	 ...
	 //player_round_score[player - 1][round_num] = player_round_score[player - 1][round_num - 1] + spin_total;
	 add_player_round_score(round_num, round_num, player, spin_total, player_scores, player_round_score, player_total);
	 //player_total[player - 1][0] = player_round_score[player - 1][10];
...
/*
   ...
   if (player_scores[player - 1][0] == 'X' && player_round_score[player - 1][2] > 0)
   player_round_score[player - 1][0] = player_round_score[player - 1][2];
   else if (player_scores[player - 1][0] == 'X' && player_round_score[player - 1][1] > 0)
   player_round_score[player - 1][0] = player_round_score[player - 1][1];
   else if (player_scores[player - 1][1] == '/' && player_round_score[player - 1][1] > 0)
   player_round_score[player - 1][0] = player_round_score[player - 1][1];
   else if (round_number == 0)
   player_round_score[player - 1][0] = spin_total;

   return player_round_score[player - 1][0];
   ...
   if (player_scores[player - 1][(round_num - 1)*2] == 'X' || player_scores[player - 1][(round_num - 1)*2 + 1] == '/') {
   player_round_score[player - 1][round_num] = add_player_round_score(--round_num, round_number, player, spin_total, player_scores, player_round_score, player_total) + spin_total;
   }
   }
   else if (round_num > 1) {

   if (player_scores[player - 1][(round_num - 2)*2] == 'X') {
   player_round_score[player - 1][round_num] = add_player_round_score(round_num - 2, round_number, player, spin_total, player_scores, player_round_score, player_total) + spin_total;
   }
   }

   }
   */
// && player_scores[player - 1][(round_num-1)*2] != 'X' && player_scores[player - 1][(round_num - 1)*2+1] != '/'
...
void extra_round_scoresheet(char **&player_name, char **&player_scores, int **&player_total, int **&player_round_score, int num_players, int round_num, int player) {
   ...
   for (int i = 0; i < num_players; i++) {
	 ...
	 cout << " |  " << player_scores[i][2*j] << " " << player_scores[i][2*j+1]; 
      ...
      if (player_scores[i][20] != 'X' || player_scores[i][21] != ' ')
	 player_total[i][0] = player_round_score[i][9];
